...
export async function GET(): Promise<Response> {
  try {
    ...
    const playersWithPR = players.map((player) => {
      ...
      const allianceSoloRate = totalAllianceKills / ((totalAllianceKills + totalAllianceDeaths) || 1);
    ...
    return new Response(
      JSON.stringify({ players: playersWithPR, totalAllianceKills, totalAllianceDeaths }),
    ...
    return new Response(
      JSON.stringify({ error: errorMessage, players: [] }),
