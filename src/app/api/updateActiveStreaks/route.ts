...
export async function POST(request: NextRequest) {
  try {
    ...
    // Save to a JSON file
    const filePath = path.join(process.cwd(), 'data', 'activeStreaks.json');
    ...
    return NextResponse.json({ 
      message: 'Active streaks saved successfully',
    ...
    return NextResponse.json({ 
      error: 'Failed to save active streaks',
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ 
      error: 'Failed to load active streaks',
      details: error.message 
    }, { status: 500 });
  }
}
